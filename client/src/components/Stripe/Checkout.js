...
const Checkout = ({
  ...
  const successPayment = data => {
    toggleModal('Payment Successful');
  };

  const errorPayment = data => {
    toggleModal('Payment Error');
  };

  const { uid } = currentSignedInUser;
  const onToken = (amount, description) => (
    token // Creates the token for you, so you can send all the necessary information to backend
  ) =>
    axios
      ...
      .then(
        axios.post(
          `https://intense-stream-29923.herokuapp.com/api/database/companies/paysuccess`,
          { uid }
        )
      )
      .catch(errorPayment);
